...
In deze fase wordt een selectie gemaakt uit de eerder opgestelde longlist met mogelijke technologische oplossingen. Deze selectie wordt gemaakt op basis van een vergelijkende analyse waarin elk alternatief wordt beoordeeld aan de hand van de criteria opgesteld in fase 2, zoals efficiënt toegangsbeheer, integratie met de bestaande webapplicatie en de WordPress-website, beveiliging en kosten.

De resultaten van deze analyse worden samengevat in een tabel die de voor- en nadelen van elk alternatief inzichtelijk maakt. Aan de hand van deze tabel wordt de meest geschikte oplossing bepaald. Dit gebeurt door het alternatief te kiezen dat de beste balans biedt tussen functionaliteit, veiligheid, gebruiksvriendelijkheid en kosten, en het meest aansluit bij de specifieke behoeften van AZL.

De alternatieven die hoog scoren maar niet als beste oplossing worden geselecteerd, worden opgenomen als back-upopties in het geval de primaire keuze tijdens de Proof of Concept-fase niet aan de verwachtingen voldoet.
\subsection{Fase 5: Proof of Concept (PoC)}
In deze fase wordt een Proof of Concept (PoC) ontwikkeld op basis van de oplossing die in fase 4 als meest geschikt werd geselecteerd. Het doel is om te testen of deze oplossing daadwerkelijk in staat is de huidige problemen binnen AZL op te lossen.

De PoC bestaat uit een werkend prototype van de cloudopslagoplossing, dat als feature wordt geïntegreerd in de bestaande webapp of publieke website. Vervolgens wordt de PoC gedurende twee weken getest door de webmaster en lesgevers. Tijdens deze testperiode wordt beoordeeld of de oplossing voldoet aan de gestelde eisen en verwachtingen, zoals efficiënt toegangsbeheer, gebruiksvriendelijkheid en integratie met bestaande systemen.

Na afloop van de testperiode wordt de feedback van de gebruikers verzameld en geanalyseerd. Op basis van deze analyse worden eventuele verbeteringen aangebracht in de PoC. Het uiteindelijke doel is om te valideren of de geselecteerde oplossing daadwerkelijk een verbetering biedt ten opzichte van de huidige Dropbox-implementatie en om te bepalen of het onderzochte alternatief geschikt is voor implementatie binnen AZL.
...
De verwachte conclusie van deze bachelorproef is dat Nextcloud, dankzij zijn flexibiliteit, open-source karakter en mogelijkheden voor integratie met bestaande systemen, zowel technisch haalbaar als praktisch effectief zal zijn voor de behoeften van de zwemclub. Mocht deze oplossing echter niet voldoen aan de verwachtingen, dan zal het onderzoek zich richten op het identificeren van de tekortkomingen en de redenen waarom deze oplossing niet geschikt bleek. In dat geval zullen de back-upopties verder worden onderzocht om een alternatief te vinden dat beter aansluit bij de eisen van AZL.